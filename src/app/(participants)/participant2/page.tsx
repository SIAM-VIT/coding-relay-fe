...
const Participant2 = () => {
  ...
  useEffect(() => {
    if (!localStorage.getItem("participantid")) {
      localStorage.setItem("participantid", "2");
    }
  ...
  useEffect(() => {
    ...
    const checkEventStatus = async () => {
      ...
      try {
        ...
        const eventEndTime = eventStartTime + 3 * 60 * 1000;
  ...
  const fetchQuestions = async () => {
    try {
      ...
      if (!selectedQuestionId) {
        selectedQuestionId = (Math.floor(Math.random() * 5) + 6).toString();
  ...
  const handleClick = () => {
    if (
      confirm(
        "Are you sure? 500 points will be deducted from your team's final score."
      )
    ) {
